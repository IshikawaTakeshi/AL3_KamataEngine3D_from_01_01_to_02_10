...
void Skeleton::Update() {
 ...
 //  
	...
	if (born1Denominator != 0) {
		float npd1 = born1Numerator / born1Denominator;
		float acosNpd1 = acosf(npd1);
		if (born_[1].tip.x >= 0) {

			born_[0].rotation.x = acosNpd1 + atanf(born_[1].tip.y / born_[1].tip.x);

		} else { //xの値がマイナスの時
			born_[0].rotation.x = acosNpd1 + atanf(born_[1].tip.y / born_[1].tip.x) - std::numbers::pi_v<float>;
		}
	}


	//ボーン1の根本・先端の更新
	born_[0].tip.x = born_[0].length * cosf(born_[0].rotation.x);
	born_[0].tip.y = born_[0].length * sinf(born_[0].rotation.x);
	...
	if (born2Denominator != 0) {
		float npd2 = born2Numerator / born2Denominator;
		float acosNpd2 = acosf(npd2);
		born_[1].rotation.x = std::numbers::pi_v<float> +acosNpd2;
	}

	//ボーン2の根本・先端の更新
	born_[1].root.x = born_[0].length * cosf(born_[0].rotation.x);
	born_[1].root.y = born_[0].length * sinf(born_[0].rotation.x);
	born_[1].tip.x = born_[0].tip.x + born_[1].length * cosf(born_[0].rotation.x + born_[1].rotation.x);
	born_[1].tip.y = born_[0].tip.y + born_[1].length * sinf(born_[0].rotation.x + born_[1].rotation.x);
	...
	//ボーンの操作
	ImGui::Begin("born");
	ImGui::SliderFloat3("born1RootTranslate", &born_[0].root.x, 0.0f, 20.0f, "%.3f");
	ImGui::SliderFloat3("born1TipTranslate", &born_[0].tip.x, 0.0f, 20.0f, "%.3f");
	ImGui::SliderFloat3("born2rootTranslate", &born_[1].root.x, 0.0f, 20.0f, "%.3f");
	ImGui::SliderFloat3("born2Translate", &born_[1].tip.x, 0.0f, 20.0f, "%.3f");
	ImGui::End();
}
