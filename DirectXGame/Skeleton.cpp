...
void Skeleton::Update() {
    ...
    // ボーン1の角度の更新 (angle.x)
    float born1NumeratorX = powf(born_[1].tip.x, 2) - powf(born_[1].tip.y, 2) + powf(born_[0].length, 2) - powf(born_[1].length, 2);
    float born1DenominatorX = 2 * born_[0].length * sqrtf(powf(born_[1].tip.x, 2) + powf(born_[1].tip.y, 2));
    
    if (born1DenominatorX != 0) {
        if (born_[1].tip.x >= 0) {
    ...
    // ボーン2の角度の更新 (angle.x)
    float born2NumeratorX = powf(born_[0].length, 2) + powf(born_[1].length, 2) - (powf(born_[1].tip.x, 2) + powf(born_[1].tip.y, 2));
    float born2DenominatorX = 2 * born_[0].length * born_[1].length;
    born_[1].angle.x = std::numbers::pi_v<float> +acos(born2NumeratorX / born2DenominatorX);
